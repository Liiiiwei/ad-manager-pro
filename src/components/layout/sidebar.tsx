    ...
    <>
      ...
      <div className="p-5 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-bold text-white leading-tight">Ad Manager Pro</h1>
            <p className="text-[10px] text-sidebar-text leading-tight">廣告數據優化工具</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 p-3 space-y-1">
        <p className="px-3 pt-2 pb-1.5 text-[10px] font-semibold text-sidebar-text/50 uppercase tracking-widest">主選單</p>
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <span className={isActive ? "text-accent" : ""}>{icons[item.icon]}</span>
              {item.label}
              {isActive && (
