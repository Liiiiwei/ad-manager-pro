...
## 六、開放決策點（已給預設，請 review 時一次拍板）

1. **配速超支門檻**：預設 `>110%` warning、`>125%` critical。要更鬆/更緊？
2. **快照頻率**：預設跟每日摘要綁（一天一次）。夠嗎？還是要盤中也偵測平台變更？
3. **操作紀錄範圍**：本期只記「預算數字變更」。要不要也偵測/記錄「暫停/開啟 campaign」？（會增加 snapshot 維度，建議 Phase 2）
4. **待辦 reason 類型**：本期只做 `pacing_overspend`。是否要加「預算剩餘見底」「花費驟降（可能投放中斷）」等？（建議先上線一種驗證閉環，再擴充）
